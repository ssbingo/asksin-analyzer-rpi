...
* (ssbingo) Die Fallunterscheidung in den Alarmmeldungen fragte das falsche Etikett ab (`rulename`), das Grafana bei Fehlalarmen gar nicht setzt — der Text fiel deshalb immer in den Normalfall. Gemessen an einer echten Meldung: Grafana setzt `alertname` auf `DatasourceError` beziehungsweise `DatasourceNoData`. Darauf wird jetzt geprüft.

### v1.1.3 — 21.08.2026

* (ssbingo) Die Alarmmeldungen unterscheiden jetzt **drei** Fälle statt zwei: echter Alarm (mit Standort), **„keine Daten"** (Abfrage lief, brachte nichts) und **„Abfrage fehlgeschlagen"**. Die beiden letzten fielen bisher in einen Topf — und genau das lässt offen, welche Arbeit ansteht: nachsehen, warum nichts ankommt, oder nachsehen, warum Grafana nicht abfragen kann.
...
* (ssbingo) Der Grafana-Knopf steht jetzt links neben den Anzeigen statt dahinter — die Statuspunkte bleiben am rechten Rand zusammen.
...
**Die Punkte im Übersichtsdiagramm tragen jetzt die Bewertung der Telegrammliste.**

- Grün ab −65 dBm, orange bis −85, rot darunter — dieselben drei Stufen wie in
  der RSSI-Spalte der Liste, aus derselben Tabelle.
- Die Legende ist anklickbar: eine Stufe ausblenden, um die schwachen
  Telegramme allein zu sehen.
- Neue maschinelle Prüfung (`tools/pruefe-rssi-stufen.py`): Sie schlägt an,
  wenn Liste oder Diagramm eigene Schwellen bekommen.
...
**Beide Funknetze auf einen Blick — am Gerät und in der Oberfläche.**

- **Neue Display-Seite 3: Funkstatus.** BidCoS links, Zigbee rechts, getrennt
  durch einen Strich, je mit Haken oder Kreuz. Zwei eigens gezeichnete
  Sinnbilder: eine Scheibe mit ausgespartem Z für Zigbee, ein Sender mit
  beidseitigen Bögen für BidCoS.
- **Zweiter Punkt in der Kopfzeile**: „Zigbee verbunden / getrennt", neben dem
  BidCoS-Punkt — und nur, wo ein Mithörer eingeschaltet ist.
- `/api/health` unterscheidet jetzt `zigbee` (eingeschaltet) von
  `zigbeeVerbunden` (der Stick antwortet auch).
- Vier Zigbee-Kacheln in einer zweiten Reihe der Übersicht.
- Beide Handbücher liegen unter eigenen Routen; auf der Zigbee-Seite steht
  jetzt auch das Zigbee-Handbuch.
